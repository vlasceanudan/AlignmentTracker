require([
  ...
  function buildSharePayload() {
    ...
    const secondaryLabel = isTrackMode ? "Offset" : "Distance";
    ...
    const lines = ["Alignment Tracker - " + date];
    if (selectedAlignment) {
      lines.push("Project: " + selectedAlignment.project);
      lines.push("Alignment: " + selectedAlignment.name);
    }
    ...
    if (lastWGS84) {
      ...
      lines.push("Coords: " + coordsText + accuracy);
  ...
  function updateShareSummary() {
    ...
    if (!totalCount) {
      shareSelectionSummaryEl.textContent = "No photos captured yet.";
    } else {
      shareSelectionSummaryEl.textContent = selectedCount + " of " + totalCount + " selected";
    }

    if (!navigator.share) {
      shareSupportNoteEl.textContent = "Native sharing is unavailable here. The app will fall back to a text-only share link.";
    } else if (selectedFiles.length && !canShareFiles(selectedFiles)) {
      shareSupportNoteEl.textContent = "This browser may share the tracker text but not attach the selected photos.";
    } else if (!selectedFiles.length) {
      shareSupportNoteEl.textContent = "Take one or more photos, then tap the ones you want to include.";
    } else {
      shareSupportNoteEl.textContent = "Selected photos will be shared together with the current tracker details.";
    }
  ...
  function autoSelectClosestAlignment(userPoint) {
    ...
    setStatus("Auto-selected: " + alignment.name + " (nearest to your location)");
